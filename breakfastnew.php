...
<body>
    ...
    <div id="modal15-breakfast-id" class="modal-breakfast">
        <div class="modal-breakfast-textbox">
			...
			$con = mysqli_connect("localhost","root","root","peach'sdiner");
				...
				$readbreakfastquery = "SELECT * FROM breakfast";
				$breakfast_results = mysqli_query($con,$readbreakfastquery);

			if (!mysqli_query($con,$readbreakfastquery))
					{
						die('Error: ') . mysqli_error($con);
					} 	

			while($breakfast_row = mysqli_fetch_array($breakfast_results)) 

			{

            echo '<table id="breakfast-table-id">';
				echo '<form action="breakfastnew.php" method="post" enctype="multipart/form-data">';
                echo '<div class="breakfast-dish-container">';
                echo '<tr>';
                  echo '<td rowspan="5" id="breakfast-dish-image-display-id" class="breakfast-display"><img class="breakfast-image" id="image" src="../Peach\'s Diner/img/' . $breakfast_row['Breakfast_Image'] . '" alt="Breakfast dish image" /></td>';
				  echo '</tr>';
				  echo '<tr>';
				  echo '<td class="breakfast-display" id="breakfast-dish-display-id">'. $breakfast_row['Breakfast_Name'] . '</td>';
                    echo '<td class="price breakfast-display" id="breakfast-price-display-id">'. $breakfast_row['Breakfast_Price'] . '</td>';
					echo '</td>';
                echo '</tr>';
                echo '<tr>';
				echo '<td class="breakfast-display" id="breakfast-description-display-id">' . $breakfast_row['Breakfast_Description'] . '</td>';
				
				$breakfastid = $breakfast_row['Breakfast_ID'];
				if (isset($_SESSION['admin'])) {
				echo '<td><i class="fas fa-edit" title="Edit" onclick="openedit('.$breakfastid.')" id="edit-id"></i><i class="fas fa-trash-alt" title="Delete" id="trash-id" onclick="openmodal('.$breakfastid.');"></i></td>';
                } else {
					echo '<td></td>';
				}
				echo '</tr>';
	...
	<div class="modal-editbreakfast-textbox">
		<form action="breakfastnew.php" method="post" enctype="multipart/form-data">
	...
	if ((!file_exists($_FILES["updatefile"]["tmp_name"])) || empty($breakfastname) || empty($breakfastdescription) || empty($breakfastprice)) {
		...
		$updatebreakfastquery = "UPDATE breakfast SET Breakfast_Name='$breakfastname', Breakfast_Description='$breakfastdescription', Breakfast_Price='$breakfastprice', Breakfast_Image='$updateimage' WHERE Breakfast_ID = '$updatebreakfastvalue'";
	...
	if ((!file_exists($_FILES["file"]["tmp_name"])) || empty($breakfastname) || empty($breakfastdescription) || empty($breakfastprice)) {
		...
		echo "<script>alert('Only image below 5MB is allowed.');</script>";
		 ...
		 if (!(move_uploaded_file($_FILES["file"]["tmp_name"], $target_file))) {
			...
			$insertbreakfastquery = "INSERT INTO breakfast (Breakfast_Name, Breakfast_Description, Breakfast_Price, Breakfast_Image, Account_ID) VALUES ('$breakfastname', '$breakfastdescription', '$breakfastprice', '$image', '$userid')";
	...
	<div class="modal-cancelbooking-textbox">
		...
		<form action="breakfastnew.php" method="post">
    ...
    if (!(password_verify($delpassword,$users['Account_Password']))) {
		...
		$deletebreakfastquery = "DELETE FROM breakfast WHERE Breakfast_ID = '$delbreakfastvalue'";
		mysqli_query($con,$deletebreakfastquery);

		if(mysqli_affected_rows($con)>0) 
		{
		echo "<script>window.alert(\"Your menu has been deleted.\");</script>";
		echo "<script>window.location.href='breakfastnew.php'</script>";
		} else {
