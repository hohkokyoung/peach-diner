...
<body style="background-image: url(img/drinks.png);">
	  ...
	  <div id="modal15-breakfast-id" class="modal-breakfast">
        <div class="modal-breakfast-textbox">
			...
			$con = mysqli_connect("localhost","root","root","peach'sdiner");
				...
				$readdrinksquery = "SELECT * FROM drinks";
				$drinks_results = mysqli_query($con,$readdrinksquery);

			if (!mysqli_query($con,$readdrinksquery))
					{
						die('Error: ') . mysqli_error($con);
					} 	

			while($drinks_row = mysqli_fetch_array($drinks_results)) 

			{  echo '<table id="breakfast-table-id">';
				echo '<form action="drinksnew.php" method="post" enctype="multipart/form-data">';
                echo '<div class="breakfast-dish-container">';
                echo '<tr>';
                  echo '<td rowspan="5" id="breakfast-dish-image-display-id" class="breakfast-display"><img class="breakfast-image" id="image" src="../Peach\'s Diner/img/' . $drinks_row['Drink_Image'] . '" alt="Breakfast dish image" /></td>';
				  echo '</tr>';
				  echo '<tr>';
				  echo '<td class="breakfast-display" id="breakfast-dish-display-id">'. $drinks_row['Drink_Name'] . '</td>';
                    echo '<td class="price breakfast-display" id="breakfast-price-display-id">'. $drinks_row['Drink_Price'] . '</td>';
					echo '</td>';
                echo '</tr>';
                echo '<tr>';
				echo '<td class="breakfast-display" id="breakfast-description-display-id">' . $drinks_row['Drink_Description'] . '</td>';
				
				$drinksid = $drinks_row['Drink_ID'];
				if (isset($_SESSION['admin'])) {
				echo '<td><i class="fas fa-edit" title="Edit" onclick="openedit('.$drinksid.')" id="edit-id"></i><i class="fas fa-trash-alt" title="Delete" id="trash-id" onclick="openmodal('.$drinksid.');"></i></td>';
                } else {
					echo '<td></td>';
				}
				echo '</tr>';
	...
	<div class="modal-editbreakfast-textbox">
		<form action="drinksnew.php" method="post" enctype="multipart/form-data">
	...
	<div class="modal-cancelbooking-textbox">
		...
		<form action="drinksnew.php" method="post">
	...
	if ((!file_exists($_FILES["file"]["tmp_name"])) || empty($breakfastname) || empty($breakfastdescription) || empty($breakfastprice)) {
		...
		echo "<script>alert('Only image below 5MB is allowed.');</script>";
		 ...
		 if (!(move_uploaded_file($_FILES["file"]["tmp_name"], $target_file))) {
			...
			$insertbreakfastquery = "INSERT INTO drinks (Drink_Name, Drink_Description, Drink_Price, Drink_Image, Account_ID) VALUES ('$breakfastname', '$breakfastdescription', '$breakfastprice', '$image', '$userid')";

			if (mysqli_query($con,$insertbreakfastquery)) {

			echo "<script>alert(\"A menu has been successfully added.\");
			window.location.href='drinksnew.php';
			</script>";
	...
	if ((!file_exists($_FILES["updatefile"]["tmp_name"])) || empty($breakfastname) || empty($breakfastdescription) || empty($breakfastprice)) {
		...
		$updatebreakfastquery = "UPDATE drinks SET Drink_Name='$breakfastname', Drink_Description='$breakfastdescription', Drink_Price='$breakfastprice', Drink_Image='$updateimage' WHERE Drink_ID = '$updatebreakfastvalue'";
    ...
    if (!(password_verify($delpassword,$users['Account_Password']))) {
		...
		$deletebreakfastquery = "DELETE FROM drinks WHERE Drink_ID = '$delbreakfastvalue'";
		mysqli_query($con,$deletebreakfastquery);

		if(mysqli_affected_rows($con)>0) 
		{
		echo "<script>window.alert(\"Your menu has been deleted.\");</script>";
		echo "<script>window.location.href='drinksnew.php'</script>";
		} else {
