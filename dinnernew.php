...
<body style="background-image: url(img/dinner-home2.png);">
	  ...
	  <div id="modal15-breakfast-id" class="modal-breakfast">
        <div class="modal-breakfast-textbox">
			...
			$con = mysqli_connect("localhost","root","root","peach'sdiner");
				...
				$readdinnerquery = "SELECT * FROM dinner";
				$dinner_results = mysqli_query($con,$readdinnerquery);

			if (!mysqli_query($con,$readdinnerquery))
					{
						die('Error: ') . mysqli_error($con);
					} 	

			while($dinner_row = mysqli_fetch_array($dinner_results)) 

			{  echo '<table id="breakfast-table-id">';
				echo '<form action="dinnernew.php" method="post" enctype="multipart/form-data">';
                echo '<div class="breakfast-dish-container">';
                echo '<tr>';
                  echo '<td rowspan="5" id="breakfast-dish-image-display-id" class="breakfast-display"><img class="breakfast-image" id="image" src="../Peach\'s Diner/img/' . $dinner_row['Dinner_Image'] . '" alt="Breakfast dish image" /></td>';
				  echo '</tr>';
				  echo '<tr>';
				  echo '<td class="breakfast-display" id="breakfast-dish-display-id">'. $dinner_row['Dinner_Name'] . '</td>';
                    echo '<td class="price breakfast-display" id="breakfast-price-display-id">'. $dinner_row['Dinner_Price'] . '</td>';
					echo '</td>';
                echo '</tr>';
                echo '<tr>';
				echo '<td class="breakfast-display" id="breakfast-description-display-id">' . $dinner_row['Dinner_Description'] . '</td>';
				
				$dinnerid = $dinner_row['Dinner_ID'];
				if (isset($_SESSION['admin'])) {
				echo '<td><i class="fas fa-edit" title="Edit" onclick="openedit('.$dinnerid.')" id="edit-id"></i><i class="fas fa-trash-alt" title="Delete" id="trash-id" onclick="openmodal('.$dinnerid.');"></i></td>';
                } else {
					echo '<td></td>';
				}
				echo '</tr>';
	...
	<div class="modal-editbreakfast-textbox">
		<form action="dinnernew.php" method="post" enctype="multipart/form-data">
	...
	<div class="modal-cancelbooking-textbox">
		...
		<form action="dinnernew.php" method="post">
	...
	if ((!file_exists($_FILES["file"]["tmp_name"])) || empty($breakfastname) || empty($breakfastdescription) || empty($breakfastprice)) {
		...
		echo "<script>alert('Only image below 5MB is allowed.');</script>";
		 ...
		 if (!(move_uploaded_file($_FILES["file"]["tmp_name"], $target_file))) {
			...
			$insertbreakfastquery = "INSERT INTO dinner (Dinner_Name, Dinner_Description, Dinner_Price, Dinner_Image, Account_ID) VALUES ('$breakfastname', '$breakfastdescription', '$breakfastprice', '$image', '$userid')";

			if (mysqli_query($con,$insertbreakfastquery)) {

			echo "<script>alert(\"A menu has been successfully added.\");
			window.location.href='dinnernew.php';
			</script>";
	...
	if ((!file_exists($_FILES["updatefile"]["tmp_name"])) || empty($breakfastname) || empty($breakfastdescription) || empty($breakfastprice)) {
		...
		$updatebreakfastquery = "UPDATE dinner SET Dinner_Name='$breakfastname', Dinner_Description='$breakfastdescription', Dinner_Price='$breakfastprice', Dinner_Image='$updateimage' WHERE Dinner_ID = '$updatebreakfastvalue'";
    ...
    if (!(password_verify($delpassword,$users['Account_Password']))) {
		...
		$deletebreakfastquery = "DELETE FROM dinner WHERE Dinner_ID = '$delbreakfastvalue'";
		mysqli_query($con,$deletebreakfastquery);

		if(mysqli_affected_rows($con)>0) 
		{
		echo "<script>window.alert(\"Your menu has been deleted.\");</script>";
		echo "<script>window.location.href='dinnernew.php'</script>";
		} else {
